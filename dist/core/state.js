 ...
 */
function createQuantar(owner, index, position) {
    return {
 ...
 */
function createCore(owner, position) {
    return {
        owner,
        position: { ...position },
        hp: CORE_HP,
...
export function createInitialState() {
    const quantarsA = INITIAL_POSITIONS.quantars.A.map((pos, i) => createQuantar("A", i, pos));
    const quantarsB = INITIAL_POSITIONS.quantars.B.map((pos, i) => createQuantar("B", i, pos));
    return {
        turn: 1,
 ...
 */
export function getEntityAt(state, position) {
    // Check Quantars
